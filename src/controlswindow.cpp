...
void ControlsWindow::createActions()
{
   connect(ui.close_pushButton, SIGNAL(clicked()), this, SLOT(close()));

   connect(ui.scanInterval_spinBox, SIGNAL(valueChanged(int)),
           this, SLOT(setScanInterval(int)));

   connect(ui.templateSize_spinBox, SIGNAL(valueChanged(int)),
           this, SLOT(setTemplateSize(int)));


}

void ControlsWindow::setEnableSpinBoxes(bool bEnable)
{
   ...
   if(bEnable)
      ...
      ui.scanInterval_spinBox->setMaximum(m_parentWindow->m_rightImage->width());
      // Sets maximum value of templateSize_spinBox according to the right image's width
      ui.templateSize_spinBox->setMaximum(m_parentWindow->m_rightImage->width());
      }
}

// begin Q_SLOTS vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
void ControlsWindow::setScanInterval(int nValue)
{
   m_nScanInterval = nValue;
}

void ControlsWindow::setTemplateSize(int nValue)
{
