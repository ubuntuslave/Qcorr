...
void
ImgLabel::setSelectable(bool bHasRubberBand)
{
         ...
         if (m_bMouseInTemplateRegion)
            {
               ...
               if (m_mousePosPoint1.x() < 0)
               ...
               else
                  {
                     ...
                     displayCoordinatesOnStatusLabel(newOriginPoint,
                           movedFinalPoint);
         ...
         else if (m_bMouseAtTemplateTopEdge)
            {
               ...
               if ((newOriginPoint.y() >= m_labelUpperLeftCornerPoint.y())
                  ...
                  {
                     m_rubberBand->setGeometry(QRect(newOriginPoint,
                           m_finalPoint).normalized());
                     displayCoordinatesOnStatusLabel(newOriginPoint,
                           m_finalPoint);
                  }
            }
         else if (m_bMouseAtTemplateBottomEdge)
            {
               ...
               if ((newOriginPoint.y() >= m_labelUpperLeftCornerPoint.y())
                     && (newOriginPoint.y() <= m_labelLowerRightCornerPoint.y()))
                  {
                     m_rubberBand->setGeometry(QRect(m_originPoint,
                           newOriginPoint).normalized());
                     displayCoordinatesOnStatusLabel(m_originPoint,
                           newOriginPoint);
                  }
            }
            ...
            { // Making a selection with the rubber band
               ...
               m_rubberBand->setGeometry(QRect(m_currentPressedPoint,
                     m_finalPoint).normalized());
               setTemplateFlags(false);
               displayCoordinatesOnStatusLabel(m_currentPressedPoint,
                     m_finalPoint);
            }
      }
   else
      {
         if (m_rubberBand->isVisible())
         ...
         else
            {
               ...
               m_currentPressedPoint = event->pos();
               displayCoordinatesOnStatusLabel(m_currentPressedPoint,
                     m_currentPressedPoint); // Show mouse moving position only
               setTemplateFlags(false);
            }
      }
}
...
void
ImgLabel::setTemplateFlags(bool bStatus)
{
...
void
ImgLabel::checkTemplateRegions(int mouseX, int mouseY)
{
   ...
   if (((mouseX > m_originPoint.x()) && (mouseY > m_originPoint.y()) && (mouseX
         < m_finalPoint.x()) && (mouseY < m_finalPoint.y())) || // When the region was selected from bottom to top
...
void
ImgLabel::displayCoordinatesOnStatusLabel(QPoint &point0, QPoint &point1)
{
   if (m_bHasRubberBand) // Only display the mouse pointer coordinate information if rubberBand actions are enabled
      {
         m_parentWindow->m_status_label->setText("Template: ("
               + QString::number(point0.x()) + ", " + QString::number(
               point0.y()) + ")px " + "<b> to </b>" + " (" + QString::number(
               point1.x()) + ", " + QString::number(point1.y()) + ")px");
      }
}
